        ...
        .then(w => {
            if (w.current) { 
                ...
                if (labelEl) {
                    ...
                    const iconEl = cardEl ? cardEl.querySelector('i') : null;
                    if (currentTemp >= 40) {
                        labelEl.innerText = "Extreme Heat Risk";
                        if (cardEl) cardEl.className = "kpi-card red";
...
function fetchLocalData() {
    if (!assignedBrgy) {
        ...
        if (tbl) tbl.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#d32f2f; padding: 40px; font-weight: bold;">No Barangay Assigned to this account.</td></tr>';
        return;
    }
    ...
    fetch(`${API_PATH}?action=master_sync&brgy=${encodeURIComponent(assignedBrgy)}`) 
        ...
        .catch(err => { 
            if (err.message !== "PHP Output was not JSON.") console.error("Sync Error:", err); 
        });
    ...
    fetch(`dashboard.php?ajax_kpi=${encodeURIComponent(assignedBrgy)}`)
        ...
        .then(data => {
            const actKpi = document.getElementById('live-kpi-active');
            const depKpi = document.getElementById('live-kpi-deployed');
            const evacKpi = document.getElementById('live-kpi-evac');
            if (actKpi) actKpi.innerText = data.active;
            if (depKpi) depKpi.innerText = data.deployed;
            if (evacKpi) evacKpi.innerText = data.evac;

            const actDet = document.getElementById('kpi-active-details');
            if (actDet) actDet.innerHTML = data.active_details && data.active_details.length ? data.active_details.join('') : '<div style="background:none; border:none; text-align:center; opacity:0.6; padding:10px;">All clear. No active incidents.</div>';
            
            const depDet = document.getElementById('kpi-deployed-details');
            if (depDet) depDet.innerHTML = data.deployed_details && data.deployed_details.length ? data.deployed_details.join('') : '<div style="background:none; border:none; text-align:center; opacity:0.6; padding:10px;">No response teams active.</div>';
            
            const evacDet = document.getElementById('kpi-evacuees-details');
            if (evacDet) evacDet.innerHTML = data.evac_details && data.evac_details.length ? data.evac_details.join('') : '<div style="background:none; border:none; text-align:center; opacity:0.6; padding:10px;">All evacuation centers empty.</div>';
        })
        ...
        fetch(API_PATH, { method: 'POST', body: fd })
            ...
            .then(d => {
                if (d.success) {
                    ...
                    customAlert("Recall Failed", d.message || "Could not recall units.", "bx-error", "#d32f2f");
                }
            })
            .catch(err => {
                console.error("Recall network error:", err);
                fetchLocalData();
            });
    });
}
        ...
        fetch(API_PATH, { method: 'POST', body: fd })
            ...
            .then(d => {
                if (d.success) {
                    ...
                    customAlert("Error", d.message || "Could not recall backup.", "bx-error", "#d32f2f");
                }
            })
            .catch(err => {
                console.error("Recall city backup error:", err);
                fetchLocalData();
            });
    });
}
...
function requestBackup(id) {
    customConfirm("Request Backup?", "Alert City Superadmin?", "bxs-error-circle", "#f57c00", function() {
        let fd = new FormData(); 
        fd.append('action', 'request_backup'); 
        fd.append('incident_id', id);
        fetch(API_PATH, { method: 'POST', body: fd })
            .then(r => r.json())
            .then(d => { if (d.success) fetchLocalData(); });
...
function cancelEscalation(incidentId) {
    customConfirm("Cancel Backup Request?", "Dismiss this backup request and return the incident to normal status?", "bx-undo", "#d32f2f", function() {
        ...
        fetch(API_PATH, { method: 'POST', body: fd })
            ...
            .then(data => {
                if (data.success) {
                    customAlert("Backup Dismissed", "Backup request has been cancelled.", "bx-check", "#388e3c");
                    fetchLocalData();
                } else {
                    customAlert("Error", data.message || "Failed to cancel backup.", "bx-error", "#d32f2f");
                }
            })
            .catch(err => {
                customAlert("Network Error", err.message, "bx-error", "#d32f2f");
